    ...
    xhr.onload = function(e) {
     ...
     if (("" + xhr.status).indexOf("2")==0) {
      ...
      //if (undefined != content && null != content) {
        ...
        if (undefined == responseType) {
          responseType = ""; // for indexof later
        }
        console.log("XHR2: Response Content type: " + responseType);
        if (undefined != xml) {
          res.format = "xml";
          wibble = xml;
        } else {
          ...
          if (xhr.status == 201 /* created */ || xhr.status == 204 /* updated */) {
            ...
            if (-1 != responseType.indexOf("application/xml") || -1 != responseType.indexOf("text/html")) { // html is a hack for 7.0-1 returning wrong content type
              xml = true;
            }
