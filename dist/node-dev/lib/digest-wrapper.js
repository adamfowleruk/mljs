 ...
 */
DigestWrapper.prototype.request = function(options, content, callback_opt) {
  this.logger.debug("DigestWrapper.request()");
  ...
  var doRequest = function() {
    digestWrapper.logger.debug("in doRequest()");
    ...
    if (content instanceof mp) {
      digestWrapper.logger.debug("in doRequest(): Got formdata");
      options.headers = content.getHeaders();
      // workaround for mime type
      //options.headers["content-type"] = options.headers["content-type"].replace(/form-data/g,"mixed");
    }
    digestWrapper.logger.debug("in doRequest(): Content type header now: " + options.headers["Content-type"]);
    ...
    digestWrapper.logger.debug("in doRequest(): setting finalReq on wrapper");
    reqWrapper.finalReq = finalReq;
    digestWrapper.logger.debug("in doRequest(): calling finaliseRequest on wrapper");
    reqWrapper.finaliseRequest();
    digestWrapper.logger.debug("in doRequest(): finaliseRequest complete");
...
/*
  ...
  if (undefined != this.realm) {
    ...
    var self = this;
    this.logger.debug("DigestWrapper: calling http.get for auth");
    var get = http.get(myopts,function(res) {
      self.logger.debug("DigestWrapper: http.get has completed in order to carry out authentication");
      //self.logger.debug("Check: " + res.statusCode);
      res.on('end', function() {
        self.logger.debug("DigestWrapper: auth http.get response end event raised");
        // check if http 401
        ...
        // E.g. from ML REST API:  Digest realm="public", qop="auth", nonce="5ffb75b7b92c8d30fe2bfce28f024a0f", opaque="b847f531f584350a"
                ...
                if (403 == res.statusCode) {
                      self.logger.debug("DigestWrapper: 403 response");
                  ...
                  // TODO via a proxy a request could succeed UNTIL AFTER a normal request has happened - need to account for that scenario too

                  self.logger.debug("DigestWrapper: other response: " + res.statusCode);
                  var auth = res.headers["www-authenticate"];
                  self.logger.debug("HEADERS: " + JSON.stringify(res.headers));
                  if (undefined != auth) {
                  ...
                  self.logger.debug("DigestWrapper: calling doRequest()");
                  doRequest();

                }

        self.logger.debug("DigestWrapper: After response handling code");

      });

      self.logger.debug("DigestWrapper: Got response for auth request");
    ...
    get.on("error",function(e) {
      self.logger.debug("DigestWrapper: Auth request in error: " + e);
      reqWrapper.error(e);
    });
  }
  return reqWrapper;
...
RequestWrapper.prototype.finaliseRequest = function() {
  this.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest called");
  var self = this;
  if (this.ended && this.finalReq != undefined) {
    this.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest: Finalising request");

    if (this.writeData != undefined /*&& this.writeData.length > 0*/) {
      this.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest: we have data - sending it now");
      ...
      if (data instanceof mp) {
        self.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest: Got a FormData instance");
        /*data.getLength(function(err, length) {
          if (err) {
            self.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest: error reading formdata.length: " + err);
            throw new Error(err);
          }*/

          //var r = request.post("http://posttestserver.com/post.php", requestCallback);
          // hard code content type too
          //self.finalReq.setHeader("Content-type","multipart/mixed");
          self.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest: Content header sent was: " + self.finalReq.getHeader("Content-type"));
          //self.finalReq._form = data;

          data.pipe(self.finalReq);

          //self.finalReq.setHeader('content-length', length); // doesn't appear to be required on MarkLogic Server (Good too - because otherwise we get a headers already sent http error!)
          self.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest: Finished processing form data");
        //});
      } else {
        this.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest: Writing non-form-data");
    ...
    this.logger.debug("DigestWrapper.RequestWrapper.finaliseRequest: Not ended yet. Skipping for now.");
  }
